...
const int SITE_NUM = 100; // number of possible websites
const int WEIGHT_NUM = 10; //number of instances per site for distance learning
const int CLASSIFY_NUM = 10; //number of instances per site for kNN training/testing
...
int DecisionMinList(Sample *header) {
  ...
  Sample *cur = header;
  while(cur != NULL) {
    counter[cur->index/CLASSIFY_NUM]++;
    cur = cur->next;
  }
...
int KNN_Classify(int TestIndex, float **FeatureMatrix_Classify, float *weight) {
  ...
  // initialize linked list: ordered from max to min
  Sample *header = NULL;
  for(int i = 0; i < NEIGHBOUR_NUM; i++) {
    Sample *ptr = new Sample;
    ptr->distance = std::numeric_limits<float>::max();
    ptr->next = header;
    header = ptr;
  }
  ...
  for(int i = 0; i < SITE_NUM*CLASSIFY_NUM; i++) {
    ...
    // if not itself: to classify
    if(i != TestIndex) {

      int Distance = dist(FeatureMatrix_Classify[TestIndex],
            FeatureMatrix_Classify[i], weight, POWER);
      if(Distance < header->distance) {
//        printf("tag1");
        header = UpdateMinList(header, Distance, i);
//        printf("tag2");
//        fflush(stdout);
      }
  ...
  // Feature Extractions
    ...
    // prepare FeatureMatrix_Weight
		for (int cur_inst = 0; cur_inst < WEIGHT_NUM; cur_inst++) {
		  do {
      ostringstream freadnamestream;
			freadnamestream << "./data/" << cur_site << "-" << real_inst << ".feature";
