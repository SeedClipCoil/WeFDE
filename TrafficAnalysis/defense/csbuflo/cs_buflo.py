...
class CSBuFLOEndpoint:
    ...
    def __init__(self, output_trace, direction, initial_rho=0.2,
    ...
    def next_timeout(self):
        ...
        if self.defended_trace:
            for i in xrange(len(self.defended_trace)-1, -1, -1):
...
class CSBuFLO:
    ...
    def defend(self, packets):
        ...
        while running:
            ...
            # padding is done.
            if self.client.done_xmitting(self.cur_time) and \
                self.server.done_xmitting(self.cur_time):
                break
    ...
    def process_next(self):
        ...
        t_events = [time_c_read_packet, time_s_read_packet,
                    c_timeout, s_timeout]

        next_packet = min([x for x in t_events if x is not None],
                           key=lambda x: x[0])
        ...
        # Process the event
        process_event = {0: self.client_read,
                         1: self.server_read,
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print 'Usage: %s <input folder> %s <output folder>'
        sys.exit(1)
...
#    DEFENDED = 'defended/'
    # Shuai: here change to destination address 
    DEFENDED = sys.argv[2]
