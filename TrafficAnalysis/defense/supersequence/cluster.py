...
fold = "TrafficAnalysis/attack/KNN/Dataset/no_defense/"
...
#        print "done/totaldone: ", done, "/", totaldone
        vote = 0
        #make your votes
        for i in range(0, len(clus)):
            if (pointers[i] != -1):
...
##            print rootlens
        ...
        superSequenceList = list()
        clusterTransmit = [0]*len(clusters)
        for c in range(0, SUPCLUSTER_NUM):
            ...
            print "rval:"
            print rval[1]
            superSequenceList.append([])
            superSequenceList[-1] = rval

            sseq = rval[0] 
            clusterlens = []
            for i in clus:
                transmitCost = transmit(sizes[i], sseq)
                clusterTransmit[i] = transmitCost
                clusterlens.append([transmitCost, i/INST_NUM])
                ...
                if (finished == 1):
                    ...
                    for i in range(0, SITE_NUM):
                        if clusters[i* INST_NUM] == c:
                            ...
                            sitecount.append(-1)
            print(clusterlens)
        return [superSequenceList, clusterTransmit] 
...
#                clusters[minsite * INST_NUM + inst] = c
                clusters[minsite * INST_NUM: minsite * INST_NUM + INST_NUM] = [c]*INST_NUM
                tocluster -= 1
        # generate supersequence
        superSequenceList = list()
        clusterTransmit = [0]*len(clusters)
        ...
        for c in range(0, CLUSTER_NUM):
            ...
            for sinste in range(0, SITE_NUM*INST_NUM):
                if (clusters[sinste] == c):
                    clus.append(sinste)
            print "generate superseq ", c
            sup = superseq(clus)
            superSequenceList.append(sup)

            # set clusterTransmit
            for each in clus:
                clusterTransmit[each] = len(sup[0])
...
SUPCLUSTER_NUM = int( sys.argv[2] )
STOPPOINTS_NUM = int( sys.argv[3]  )
method =  int( sys.argv[4] )

returnVal = ucluster(method)
superSequenceList = returnVal[0]
clusterTransmit = returnVal[1]
print "method: ", method
print "clusters: ", clusters
print "stop points: ", stoppoints
print "superSequenceList: " 
pprint.pprint(superSequenceList)
print "clusterTransmit", clusterTransmit


#TraceManager

if method == 4:
    # method 4: initiate stoppoints to be length of superseq
    for c in range(CLUSTER_NUM):
        superLen = len( superSequenceList[c][0] )
        stoppoints.append([superLen])



tm = TraceManager(sizes, times, SITE_NUM, INST_NUM)
tm.setGenerator(superSequenceList, clusters, stoppoints, method)
tm.setDstPath('TrafficAnalysis/attack/KNN/Dataset/defenses/supersequence_method' + str(method) + 'supercluster'+ str(SUPCLUSTER_NUM) + '_clusternum' + str(CLUSTER_NUM) + '_stoppoints' + str(STOPPOINTS_NUM) + '/')
tm.run(clusterTransmit)
